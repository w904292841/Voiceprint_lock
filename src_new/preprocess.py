...
for id, l in enumerate(labellist):
    if l == '1' and int(namelist[id][2:7]) <= 10128:
        mode = 'train'
    elif l == '2' and int(namelist[id][2:7]) <= 10128:
        mode = 'dev'
    elif int(namelist[id][2:7]) <= 10128:
        mode = 'test'
    else:
        continue
    ...
    for j in range(data.size(0)):
        this_data = data[j]
        real_i += 1
        torch.save(this_data, write_path + mode + "/" + namelist[id][:7] + "_" + str(real_i) + ".pt")
